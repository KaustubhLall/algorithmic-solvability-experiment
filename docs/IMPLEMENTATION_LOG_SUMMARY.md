...
>
> **Last Updated:** 2026-03-27 (TASK-20 complete; classification baseline-separation repaired, affected experiments rerun, publication assets refreshed, full suite 465 passed / 17 warnings)
...
| TASK-04 | Task Registry (SR-1) | 2025-03-25 | [log](implementation_log/TASK-04_task_registry.md) | **FOUNDATION MILESTONE.** 32 tasks across S0-S3 and C0-C3 in `src/registry.py`. 36 V-1 tests pass. |
| TASK-05 | Data Generator (SR-3) | 2025-03-25 | [log](implementation_log/TASK-05_data_generator.md) | Label re-verification, noise injection, and multi-task generation in `src/data_generator.py`. 24 V-3 tests pass. |
...
| TASK-10 | Report Generator (SR-8) | 2026-03-25 | [log](implementation_log/TASK-10_report_generator.md) | Structured experiment artifact writing, per-task metrics/errors JSON, markdown summaries, plots, and solvability verdict computation in `src/reporting.py`. 14 V-8 tests pass. |
...
| TASK-14 | Diagnostic Experiments | 2026-03-26 | [log](implementation_log/TASK-14_diagnostic_experiments.md) | Added `src/diagnostic_experiments.py`, `main.py diagnostic`, `tests/test_diagnostic_experiments.py` (22 tests). Implemented EXP-D1 through EXP-D5 and exposed `InputEncoder.feature_names` plus `SklearnModelWrapper.estimator` for diagnostics. |
...
| TASK-17 | Methodology Feedback Execution | 2026-03-26 | [log](implementation_log/TASK-17_methodology_feedback_execution.md) | Centralized diagnostic-evidence resolution in SR-8, wired EXP-D1/D2/D4 into baseline solvability verdicts, aligned EXP-D5 to the same logic, reran the full `smoke`/`classification`/`sequence`/`diagnostic`/`bonus` stack, regenerated `output/publication_assets/`, and moved the classification/control baseline to 2 `STRONG` tasks with D5 now unchanged. |
| TASK-18 | Sequence Training Protocol Upgrade | 2026-03-26 | [log](implementation_log/TASK-18_sequence_training_protocol_upgrade.md) | Upgraded the LSTM sequence protocol to 200 epochs, weight decay, and `ReduceLROnPlateau`; moved `EXP-S1` and `EXP-S2` to 5 seeds and 1000 samples; logged per-epoch `training_curve` artifacts; reran `sequence`, `diagnostic`, `bonus`, `smoke`, and `classification`; and regenerated `output/publication_assets/` with a new sequence-training-dynamics figure and full 16/16 runtime coverage. |
| TASK-19 | Methodology Synthesis and Second-Paper Draft | 2026-03-26 | [log](implementation_log/TASK-19_methodology_synthesis_second_paper.md) | Rewrote the methodology review and publication-facing analysis around the refreshed TASK-18 evidence, created a submission-style second-paper manuscript in `output/pdf/`, compiled it to PDF, and QA'd the compiled pages through rendered page images plus page-structure checks so the paper package is derived from the current asset bundle rather than from stale narrative summaries. |
| TASK-20 | Classification Baseline Separation Repair | 2026-03-27 | [log](implementation_log/TASK-20_classification_baseline_separation_repair.md) | Rebalanced `C2.1_and_rule` at the sampler level in `src/registry.py`, added regression coverage in `tests/test_registry.py` and `tests/test_data_generator.py`, reran `classification`, `diagnostic`, and `bonus`, regenerated `output/publication_assets/`, and moved the classification/control baseline to 3 `STRONG`, 9 `MODERATE`, 1 `INCONCLUSIVE`, and 1 `NEGATIVE`. |
...
- **Longer LSTM training changes the sequence story, but only partially.** After TASK-18, the sequence/control benchmark moves to 11 `NEGATIVE`, 1 `MODERATE`, 2 `WEAK`, and 2 `INCONCLUSIVE`; `S1.4_count_symbol` becomes `MODERATE`, `S1.5_parity` becomes `WEAK` under the real LSTM path, and `S2.3_running_min` becomes `INCONCLUSIVE`.
...
- **The implemented classification benchmark is still much stronger than the sequence suite at current scale.** After the TASK-18 refresh, classification/control remains at 2 `STRONG`, 9 `MODERATE`, 1 `WEAK`, 1 `INCONCLUSIVE`, and 1 `NEGATIVE`, while sequence/control improves but still trails at 11 `NEGATIVE`, 1 `MODERATE`, 2 `WEAK`, and 2 `INCONCLUSIVE`.
...
- **Baseline verdicts can safely absorb diagnostic artifacts once the parsing logic is centralized.** TASK-17 moved D1/D2/D4 interpretation into SR-8 so baseline reports and EXP-D5 no longer drift.
- **A zero-upgrade calibration pass is healthy once baseline verdicts already include the same evidence.** After TASK-17, D5's value is consistency checking (`controls_negative_or_weak`, `trivial_tasks_strong`), not manufacturing extra label changes.
- **Counterfactual proxy notes need to disclose fallback models.** When EXP-D4 does not evaluate the exact best baseline model, the verdict note should say that the best available alignment model was used instead of silently implying a perfect model match.
- **Validation-loss checkpoints and held-out accuracy can diverge on harder sequence tasks.** Under the TASK-18 protocol, `S1.2_sort` and `S2.3_running_min` keep improving on held-out exact match after the internal-validation checkpoint is selected, which is scientifically informative but means the reported checkpointed IID score is deliberately conservative.
- **Publication assets should include methodology-change evidence, not just end-state verdicts.** The `sequence_training_dynamics.csv` table and `sequence_training_dynamics.png` figure make the TASK-18 upgrade interpretable in the manuscript instead of burying the training-curve evidence inside per-task `metrics.json`.
- **Runtime coverage should be treated as a first-class reproducibility requirement.** Adding runtime metadata to diagnostic and bonus summaries removed the last major completeness gap in `output/publication_assets/data/publication_summary.json`.
- **The paper-writing workflow should be asset-driven, not summary-driven.** TASK-19 worked cleanly only after rebuilding the markdown analysis from `output/publication_assets/data/` and using that analysis as the source for the manuscript structure.
- **Training-dynamics evidence changed the second-paper argument materially.** The new figure shows that easy sequence tasks saturate early while harder tasks keep improving after the validation-selected checkpoint, which supports a more precise claim than the earlier blanket "sequence fails" summary.
- **PDF QA needs to be a release gate.** Rendering compiled pages and checking page structure from the PDF catches issues that would not have been visible from raw LaTeX alone.
- **Baseline-separation problems are better fixed at the task prior than at the verdict threshold.** TASK-20 showed that `C2.1_and_rule` was methodologically weak because of its sampled class prior, not because SR-8 expected too much from a learned model. Rebalancing the task-local sampler preserved the rule and kept the reporting criteria stable.
- **Distinguish registry scope from baseline-report scope explicitly.** The repo currently registers 32 tasks, but the baseline solvability tables cover 30 because one constant control per track is outside the baseline reporting set. Writing that distinction down avoids another 32-vs-30 review comment cycle.
...
_None. TASK-20 is complete; the next queued work is TASK-21 baseline distractor split support._
...
|---|---|---|---|
| V-1 | Task Registry | **PASS** | 36 tests, all passing |
| V-2 | Input Schema | **PASS** | 54 tests, all passing |
| V-3 | Data Generator | **PASS** | 24 tests, all passing |
...
| V-8 | Report Generator | **PASS** | 14 tests, all passing |
...
| V-14 | Diagnostic Experiments | **PASS** | 22 tests, all passing |
| V-15 | Bonus Experiments | **PASS** | 20 tests, all passing |

Full suite status from the fresh TASK-20 validation pass: **465 passed, 17 warnings**. The warnings were training-stack/runtime warnings, not correctness failures.
